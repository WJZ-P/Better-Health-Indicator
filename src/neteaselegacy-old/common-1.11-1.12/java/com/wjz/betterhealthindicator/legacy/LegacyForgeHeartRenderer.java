...
/** Shared full-heart renderer for the nearly identical Forge 1.11.2 and 1.12.2 APIs. */
public final class LegacyForgeHeartRenderer {
    private static final String MOD_ID = "better_health_indicator";
    ...
    public void onAttack(AttackEntityEvent event) {
        ...
        if (event.getEntityPlayer() == minecraft.player && event.getTarget() instanceof EntityLivingBase) {
            attackedTarget = (EntityLivingBase) event.getTarget();
    ...
    public void onRenderLiving(RenderLivingEvent.Post<EntityLivingBase> event) {
        ...
        if (distanceSquared(entity, minecraft.player) > MAX_DISTANCE_SQUARED) return;
        ...
        GlStateManager.translate(event.getX(), event.getY() + entity.height + 0.45D, event.getZ());
        ...
        GlStateManager.scale(-WORLD_SCALE, -WORLD_SCALE, WORLD_SCALE);
    ...
    private static void renderHeartRow(
        ...
        for (LegacyHeartAnimation.Slot slot : view.slots) {
            ...
            GlStateManager.pushMatrix();
            GlStateManager.translate(slot.x + offset.x, offset.y, 0.0F);
            GlStateManager.rotate((float) Math.toDegrees(offset.rotation), 0.0F, 0.0F, 1.0F);
    ...
    private static void drawWorldText(
        ...
        if (view.multiplier > 0) {
            ...
            font.drawStringWithShadow(
                    multiplier,
                    rightEdge - font.getStringWidth(multiplier) - 3.0F,
                    -4.0F,
    ...
    private static void renderParticles(
        ...
        for (LegacyHeartAnimation.Particle particle : particles) {
            ...
            GlStateManager.pushMatrix();
            GlStateManager.translate(particle.renderX(partialTick), particle.renderY(partialTick), 0.02F);
            GlStateManager.rotate(
    ...
    private static void renderHud(Minecraft minecraft, EntityLivingBase target, float health, float maximum) {
        ...
        String label = target.getDisplayName().getFormattedText() + "  "
        ...
        minecraft.fontRenderer.drawStringWithShadow(label, x + 6.0F, y + 6.0F, 0xFFFFFF);
    }
